...
//    //UTILIZZO
//    /*
//    if (GlobalVar.EconomySave == true) {
//          try {
//             Economy = await S0_CapacitorEconomy(EconomyObj, GlobalVar.Page, GlobalVar.EconomySaveLog);
//             //alert("Economy inizializzato");
//          } catch (error) {
//             //alert("Errore durante l'inizializzazione dell'app: " + error);
//          };
//       }
//       else Economy = EconomyObj;
//    */
...
// async function E3_ShaderLens(Obj, ambientLight, directionalLight) {
//    const Texture1 = await E3_LoadEditTexture(null, Obj.Texture1, true);
//    const Texture2 = await E3_LoadEditTexture(null, Obj.Texture2, true);
...
//       uniforms: {
//          //texture 
//          // map1: { value: E3_LoadEditTexture(null, Obj.Texture1, true) },
//          // map2: { value: E3_LoadEditTexture(null, Obj.Texture2, true) },
//          map1: { value: Texture1 },
//          map2: { value: Texture2 },
...
//    /*-------------FUNZIONI DI MODIFICA DEL MATERIALE----------------*/
//    //VARIABILI/OGGETTI
...
//       material.uniforms.lensPosition.value.copy(lensWorldPos);
...
//       material.uniforms.lightDirection.value.copy(lightWorldPos.clone().sub(objWorldPos).normalize());
//       material.uniforms.directionalColor.value = DirectionalLight.color.clone();
...
async function E3_ShaderLens2(Obj) {
   ...
   //       .copy(lightWorldPos.sub(objWorldPos).normalize());
...
//       };
//       //AGGIORNA IL TESTO DEL PULSANTE DI RIEMPIMENTO
//       if (Obj.Value > Obj.MaxValue * (1 - Obj.Fill)) Obj.Hud.setButtonText(Obj.Pulsindex, `FILL
//          ${((Obj.MaxValue - Obj.Value) * Obj.PriceUnit).toFixed(0)}${Obj.MoneySymbol}`);
//       else Obj.Hud.setButtonText(Obj.Pulsindex, `FILL 1/${1 / Obj.Fill}
...
function E0_DynamicCockpit(Oggetto) {
      ...
      for (let i = 0; i < VarPlanetSystem.PlanetsNum + 1; i++) {
         ...
         if (DynamCockpitVar.PlanetVisible[i] == true) {
            ...
            //ALTRIMENTI
               ...
               // DynamCockpit.children[0].children[i + 1].children[1].visible = true;       //TESTO INDICATORE
         ...
         for (let i = 0; i < VarPlanetSystem.NumMoons; i++) {
            ...
            if (DynamCockpitVar.MoonVisible[i] == true) {
               ...
               //SE LA LUNA NON È LA DESTINAZIONE
                  ...
                  DynamCockpit.children[1].children[i].children[0].visible = true;     //SPRITE VISORE
                  //TESTO INDICATORE SOLO SE IN SUA DIREZIONE
                  // if ((Cockpit.children[1].children[i].rotation.x > Math.PI - Par.DynamicCockpit.VisDiff || 
                  //    Cockpit.children[1].children[i].rotation.x < -Math.PI + Par.DynamicCockpit.VisDiff) && 
                  //    Cockpit.children[1].children[i].rotation.y < Par.DynamicCockpit.VisDiff && 
                  //    Cockpit.children[1].children[i].rotation.y > -Par.DynamicCockpit.VisDiff) {
         ...
         for (let i = 0; i < VarPlanetSystem.NumSubMoons; i++) {
            ...
            if (DynamCockpitVar.SubMoonVisible[i] == true) {
               ...
               //SE LA SUB-LUNA NON È LA DESTINAZIONE
                  ...
                  DynamCockpit.children[2].children[i].children[0].visible = true;     //SPRITE VISORE
                  //TESTO INDICATORE SOLO SE IN SUA DIREZIONE
                  // if ((Cockpit.children[2].children[i].rotation.x > Math.PI - Par.DynamicCockpit.VisDiff || 
                  //    Cockpit.children[2].children[i].rotation.x < -Math.PI + Par.DynamicCockpit.VisDiff) && 
                  //    Cockpit.children[2].children[i].rotation.y < Par.DynamicCockpit.VisDiff && 
                  //    Cockpit.children[2].children[i].rotation.y > -Par.DynamicCockpit.VisDiff) {
   ...
   setInterval(() => {
      ...
      // console.log(DynamCockpitVar.PlanetVisible[0]);
   }, 1000);
...
async function E0_DynamicPlanetarySystem() {
   ...
   setInterval(async () => {
      if (VarPlanetSystem.OrbitOnceLoaded == true) {
         ...
         /*---------------------FUNZIONE AGGIORNAMENTO PROMISES DEGLI OGGETTI CREATI DALLA FUNZIONE CreateObj--------------------*/
         // if (VarPlanetSystem.PlanetOrbit === 0 && VarPlanetSystem.MinTimePlanet < Par.PlanetarySystem.Parametri.MinTimeLoadMesh && VarPlanetSystem.LastIndexTrigger !== VarPlanetSystem.NearPlanetIndex) {
         ...
         // }
         // else if (VarPlanetSystem.PlanetOrbit !== 0 || VarPlanetSystem.MinTimePlanet >= Par.PlanetarySystem.Parametri.MinTimeLoadMesh) {
         //    // Stato INATTIVO: reset garantito
         //    VarPlanetSystem.NearPlanetUpdate = false;
         // }
         // else {
         //    // Stato neutro (sotto soglia ma stesso pianeta)
         //    // IMPORTANTE: reset esplicito
         //    VarPlanetSystem.NearPlanetUpdate = false;
         // };
   ...
   // setInterval(() => {
   //    console.log(`Orbit: ${VarPlanetSystem.PlanetOrbit}, Trigger: ${VarPlanetSystem.LastIndexTrigger}, Index: ${VarPlanetSystem.NearPlanetIndex}, MinTime: ${VarPlanetSystem.MinTimePlanet}`);
   // }, 1000);
...
function E0_OrbitControl(Camera, Elem) {
   ...
   //    Camera.position.copy(this.target).addScaledVector(_dollyDir, newDistance);
...
export async function MicEngine(Parameters, OggettiObj, GeometrieObj, MaterialiObj) {
   ...
   MicEnginereturn.Materials.Export = [];
   MicEnginereturn.Materials.E3_MaterialeBase = E3_MaterialeBase;
   MicEnginereturn.Materials.E3_MaterialeBaseColor = E3_MaterialeBaseColor;
   MicEnginereturn.Materials.E3_MaterialeStandard = E3_MaterialeStandard;
   MicEnginereturn.Materials.E3_MaterialeLucido = E3_MaterialeLucido;
   MicEnginereturn.Materials.E3_MaterialeOpaco = E3_MaterialeOpaco;
   MicEnginereturn.Materials.E3_MaterialeSprite = E3_MaterialeSprite;
   MicEnginereturn.Materials.E3_MaterialePunti = E3_MaterialePunti;
   MicEnginereturn.Materials.E3_LoadEditTexture = E3_LoadEditTexture;

   MicEnginereturn.Materials.Export = MaterialExportArray;        //MATERIALI ESPORTATI

   /*--------------------------------------------MATERIALI PERSONALIZZATI THREE.JS----------------------------------------------*/
   MicEnginereturn.Materials.E3_ShaderGlow = E3_ShaderGlow;
   MicEnginereturn.Materials.E3_EditShaderGlow = E3_EditShaderGlow;
   MicEnginereturn.Materials.E3_ShaderLens = E3_ShaderLens2;
   MicEnginereturn.Materials.E3_ShaderOverlay = E3_ShaderOverlay;
   MicEnginereturn.Materials.E3_StampCanvas = E3_StampCanvas;
   ...
   MicEnginereturn.Objects.E3_GenMesh = E3_GenMesh;
   MicEnginereturn.Objects.E3_UniversalMesh = E3_UniversalMesh;
   MicEnginereturn.Objects.E3_GroupCanvasSprite = E3_GroupCanvasSprite;
   MicEnginereturn.Objects.E3_Vector3 = E3_Vector3;
   ...
   MicEnginereturn.Math.E3_DisplayTime = E3_DisplayTime;
