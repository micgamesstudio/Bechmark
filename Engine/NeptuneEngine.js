...
//       void main() {
//         // Calcolo dell’angolo in modo leggero
//         float angle = dot(vNormal, normalize(vViewPosition));
//         float intensity = max(0.0, 0.6 - angle) * glowIntensity;

//         // Pulsazione semplificata: ridotta a un solo moltiplicatore
...
//       viewVector: { value: new THREE.Vector3(1, 0, 0) },
//       glowIntensity: { value: Obj.Intensity },
//       //time: { value: 0.0 }  // ora statico, senza pulsazione automatica
//     },
//     vertexShader: `
//       varying vec3 vNormal;
//       varying vec3 vViewPosition;
...
//       uniform vec3 glowColor;
//       uniform float glowIntensity;
//       // uniform float time;  // non più usato
//       varying vec3 vNormal;
//       varying vec3 vViewPosition;
...
//             float a = clamp(0.6 - angle, 0.0, 1.0);
//             float intensity = a * a * a * glowIntensity;
...
//    glowMaterial.SetColor = function (Color) {
//       glowMaterial.uniforms.glowColor.value.setHex(Color);
//    };

//    glowMaterial.SetIntensity = function (Intensity) {
//       glowMaterial.uniforms.glowIntensity.value = Intensity;
//    };
...
//       //FRAGMENT SHADER (STABILE)
//       fragmentShader: `
//          // precision highp float;        // DATO MODIFICATO
//          precision mediump float;
...
//             // float denom = dot(lensNormal, rayDir);        // DATO MODIFICATO
//             vec3 LN = normalize(lensNormal);
//             float denom = dot(LN, rayDir);
...
//             // vec3 linearTex = pow(texColor.rgb, vec3(2.2));        // DATO MODIFICATO
//             vec3 linearTex = texColor.rgb * texColor.rgb;
...
export function S0_GenerateHUDCanvas(config, Options) {
   ...
   canvas.style.left = `${(window.innerWidth - window.innerWidth * config.Width) * 0.5}px`;
...
async function E3_Benchmark(Obj) {
   ...
   //       const key = "A" + model.toUpperCase(); // es: 642L -> A642L
   ...
   //             return {
   //                key: key,         // la chiave trovata
   //                data: group[key]  // l'oggetto con Score1, Score2
   //             };
   //          }
   //       }
   //       return null;
   ...
   //       const clean = model.toUpperCase().replace(/\s+/g, "_"); // es: "G715 MP7" -> "G715_MP7"
   ...
   //          // Immortalis con prefisso I (IG715, IG925…)
   //          const iKey = "I" + clean;
   //          if (iKey in arch) {
   //             return {
   //                key: iKey,
   ...
   if (out.evaluation.deviceType == "mobile") {
      ...
      if (Detect?.data) {
         ...
         out.detect.Bar2 = Detect.data.Score1 / out.reference.mobile[1].score;
      }
      else {
...
function E2_UpdateRotation(delta) {
   ...
   if (VarPlanetSystem.PlanetOrbit > 0) {
      // console.log(VarPlanetSystem.References[VarPlanetSystem.PlanetOrbit - 1].DayRot.name);
      // console.log(Planetary.children[VarPlanetSystem.PlanetOrbit].children[0].children[0].name);
   ...
   for (let i = 0; i < Oggetti.PlanetarySystem.Modular.length; i++) {
      ...
      for (let a = 0; a < Oggetti.PlanetarySystem.Modular[i].Modular.length; a++) {
         ...
         if (Oggetti.PlanetarySystem.Modular[i].Modular[a].Type == 0) {
            ...
            // MoonMesh.scale.setScalar(MoonScaleXZ);
   ...
   */
   ...
   function Update() {
      ...
      if (Obj.Control == 0) {
         ...
         //0 NIPPLE0X
         // Axe[Obj.VirtualAxe[0]] = MicEnginereturn.VarPad[0].ValX * ((Obj.RegAxe[Obj.VirtualAxe[0]] + 5) / 100) * Obj.InvAxe[Obj.VirtualAxe[0]];
...
export async function MicEngine(Parameters, OggettiObj, GeometrieObj, MaterialiObj) {
   ...
   MicEnginereturn.Materials = {
      ...
      // E3_EditShaderGlow: E3_EditShaderGlow,
   ...
   // MicEnginereturn.Geometries.E3_GeoBox = E3_GeoBox;
   // MicEnginereturn.Geometries.E3_GeoCylinder = E3_GeoCylinder;
   // MicEnginereturn.Geometries.E3_GeoSphere = E3_GeoSphere;
   // MicEnginereturn.Geometries.E3_GeoRing = E3_GeoRing;
   // MicEnginereturn.Geometries.E3_GeoPlane = E3_GeoPlane;
   // MicEnginereturn.Geometries.E3_GeoCircle = E3_GeoCircle;
   // MicEnginereturn.Geometries.E3_GenerateFilamentCloud = E3_GenerateFilamentCloud;
   ...
   // MicEnginereturn.Objects.E3_Group = E3_Group;
   // MicEnginereturn.Objects.E3_Object3D = E3_Object3D;
   // MicEnginereturn.Objects.E3_Quaternion = E3_Quaternion;
   // MicEnginereturn.Objects.E3_Matrix4 = E3_Matrix4;
   // MicEnginereturn.Objects.E3_Euler = E3_Euler;
   // MicEnginereturn.Objects.E3_CircularGradient = E3_CircularGradient;
   ...
   // MicEnginereturn.Math.E3_AngleZero = E3_AngleZero;
   // MicEnginereturn.Math.CompareIncrement = CompareIncrement;
   // MicEnginereturn.Math.E3_EulerQuaternionInterpolation = E3_EulerQuaternionInterpolation;
   ...
   if (Par.Moduli.Audio == true) {
      UrlModule = "Audio";
      // MicEnginereturn.Audio = E0_Audio();
