...
//ABILITARE ADMOB (POKI E CRAZYGAMES)
// export async function S0_CapacitorAdmob(BannerId, VideoId, LogAlert) {
//    let provider = "admob"; //puoi cambiare dinamicamente se vuoi
//    let videoReady = false;
//    let AdMobModule = null;

//    function log(msg) {
//       if (LogAlert) console.log("[AdWrapper] " + msg);
//    }
...
//       AdMobModule = { AdMob, BannerAdSize, BannerAdPosition, RewardAdPluginEvents };
...
//       //Evento quando il video è pronto
//       AdMob.addListener(RewardAdPluginEvents.Loaded, () => {
//          videoReady = true;
//          log("Video premiante pronto (AdMob)");
//          window.dispatchEvent(new Event("VideoPronto"));
//       });
//    }

//    /*-------------------------------------------
//        Inizializzazione CrazyGames
//    -------------------------------------------*/
//    async function initCrazyGames() {
//       if (!window.CrazyGames || !CrazyGames.SDK) {
//          throw new Error("CrazyGames SDK non trovato");
//       }
//       log("CrazyGames SDK pronto");
//    }

//    /*-------------------------------------------
//        Inizializzazione Poki
//    -------------------------------------------*/
//    async function initPoki() {
//       if (!window.PokiSDK) {
//          throw new Error("Poki SDK non trovato");
//       }
//       log("Poki SDK pronto");
//    }
...
//             margin: 0,
//          });
//          log("Banner mostrato (AdMob)");
//       } else if (provider === "crazygames") {
//          if (window.CrazyGames && CrazyGames.SDK) {
//             CrazyGames.SDK.banner.display("bottom");
//          }
//       } else if (provider === "poki") {
//          if (window.PokiSDK) {
//             PokiSDK.displayAd && PokiSDK.displayAd();
//          }
//       }
//    }

//    async function nascondiBanner() {
//       if (provider === "admob") {
//          const { AdMob } = AdMobModule;
//          await AdMob.hideBanner();
//       } else if (provider === "crazygames") {
//          if (window.CrazyGames && CrazyGames.SDK) {
//             CrazyGames.SDK.banner.hide();
//          }
//       } else if (provider === "poki") {
//          if (window.PokiSDK) {
//             PokiSDK.hideAd && PokiSDK.hideAd();
//          }
//       }
//    }
...
//        Video premiante
//    -------------------------------------------*/
//    async function caricaVideoPremiante() {
//       if (provider === "admob") {
//          const { AdMob } = AdMobModule;
//          await AdMob.prepareRewardVideoAd({ adId: VideoId });
//       } else if (provider === "crazygames") {
//          CrazyGames.SDK.ad.requestAd("rewarded", (wasLoaded) => {
//             if (wasLoaded !== false) {
//                videoReady = true;
//                window.dispatchEvent(new Event("VideoPronto"));
//                log("Video premiante pronto (CrazyGames)");
//             }
//          });
//       } else if (provider === "poki") {
//          setTimeout(() => {
//             videoReady = true;
//             window.dispatchEvent(new Event("VideoPronto"));
//             log("Video premiante pronto (Poki)");
//          }, 500);
//       }
//    }

//    async function mostraVideoPremiante() {
//       return new Promise(async (resolve, reject) => {
//          if (!videoReady) return reject({ Reward: false });
...
//                listener.remove();
//                videoReady = false;
//                resolve({ Reward: true, Data: reward });
//             });
...
//          } else if (provider === "crazygames") {
//             CrazyGames.SDK.ad.requestAd("rewarded", (wasCompleted) => {
//                videoReady = false;
//                resolve({ Reward: !!wasCompleted });
//             });
...
//    -------------------------------------------*/
//    try {
//       if (provider === "admob") await initAdmob();
//       else if (provider === "crazygames") await initCrazyGames();
//       else if (provider === "poki") await initPoki();
//    } catch (err) {
//       if (LogAlert) alert("Errore inizializzazione Ads: " + err);
//    }

//    return { mostraBanner, nascondiBanner, caricaVideoPremiante, mostraVideoPremiante };
// };
...
const GroupUser = new THREE.Group();            //GRUPPO GENERALE SPACESHIP
...
const UserPosWorld = new THREE.Vector3();       //POSIZIONE WORLD SPACESHIP
...
/*-----------------------------------------------------OGGETTI ENGINE---------------------------------------------------*/
let ObjCreateObj;    //OGGETTO RISULTATO DELLA FUNZIONE CreateObj
const Promises = [];
let MaterialArray;
const Oggetti3D = {
   Spaceship: {
...
export function S0_GenerateHUDCanvas(config, Options) {
   ...
   // function parsePos(flag, left, right, PosX, topFlag, top, bottom, PosY) {
   //    let x, y;

   //    if (flag === "Left") {
   //       if (PosX) x = parseSize(PosX, canvas.width);
   //       else x = parseSize(left, canvas.width);                     //IN FUTURO ELIMINARE
   //    } else {
   //       if (PosX) x = canvas.width - parseSize(PosX, canvas.width);
   //       else x = canvas.width - parseSize(right, canvas.width);                     //IN FUTURO ELIMINARE
   //    }

   //    if (topFlag === "Top") {
   //       if (PosY) y = parseSize(PosY, canvas.height);
   //       else y = parseSize(top, canvas.height);                     //IN FUTURO ELIMINARE
   //    } else {
   //       if (PosY) y = canvas.height - parseSize(PosY, canvas.height);
   //       else y = canvas.height - parseSize(bottom, canvas.height);                     //IN FUTURO ELIMINARE
   //    }
   ...
   function GenerateObjects() {
      ...
      for (let i = 0; i < config.Pulsanti; i++) {
         ...
         //    config.PulsPos[i].Left,
         //    config.PulsPos[i].Right,
         //    config.PulsPos[i].PosX,
         //    config.PulsPos[i].TopFlag || "Top",
         //    config.PulsPos[i].Top,
         //    config.PulsPos[i].Bottom,
      ...
      for (let i = 0; i < (config.Barre || 0); i++) {
         ...
         //    config.BarPos[i].Left,
         //    config.BarPos[i].Right,
         //    config.BarPos[i].PosX,
         //    config.BarPos[i].TopFlag || "Top",
         //    config.BarPos[i].Top,
         //    config.BarPos[i].Bottom,
      ...
      for (let i = 0; i < (config.Immagini || 0); i++) {
         ...
         //    config.ImgPos[i].Left,
         //    config.ImgPos[i].Right,
         //    config.ImgPos[i].PosX,
         //    config.ImgPos[i].TopFlag || "Top",
         //    config.ImgPos[i].Top,
         //    config.ImgPos[i].Bottom,
...
GenerateObjects();
   ...
   //    console.log(e);
   //          //canvas.setPointerCapture(e.pointerId);  // IMPORTANTE
   ...
   //    e.preventDefault();
   //    ResetHitTest();
   //    // canvas.releasePointerCapture(e.pointerId);  // CONSIGLIATO

   //    // const rect = canvas.getBoundingClientRect();
   //    // const x = (e.clientX - rect.left) * ratio;
   //    // const y = (e.clientY - rect.top) * ratio;

   //    // const handled = hitTestUp(x, y);  // pointercancel equivale a un pointerup perso
   ...
   //    //    const newEvent = new PointerEvent("pointerup", {   // pointerup simulato
   //    //       bubbles: true,
   //    //       cancelable: true,
   //    //       clientX: e.clientX,
   //    //       clientY: e.clientY,
   ...
   if (config.Events == true) {
      ...
      canvas.addEventListener('pointerdown', e => {
         ...
         const handled = hitTest(x, y);
         activePointers.set(e.pointerId, handled); // salva se il menu ha gestito il dito

         if (!handled && Options.DispatchEvent) {
            dispatchToUnderCanvas(e); // forward al renderer
         } else {
            e.preventDefault(); // blocca il comportamento nativo solo se handled
         }
      }, { passive: false });
      ...
      canvas.addEventListener('pointermove', e => {
         ...
         if (handled === false) {
            // il dito non era gestito dal menu: forwarda al renderer
            if (Options.DispatchEvent) dispatchToUnderCanvas(e);
         } else if (handled === true) {
            // il dito era gestito dal menu: puoi gestire drag dei pulsanti qui se vuoi
            e.preventDefault(); // blocca scroll/gesture nativa
         }
      }, { passive: false });
      ...
      canvas.addEventListener('pointerup', e => {
         ...
         if (handled === false) {
            if (Options.DispatchEvent) dispatchToUnderCanvas(e); // forward al renderer
         } else {
            ResetHitTest(); // menu gestito
            e.preventDefault();
         }
      ...
      canvas.addEventListener('pointercancel', e => {
         ...
         if (handled === false) {
            if (Options.DispatchEvent) dispatchToUnderCanvas(e, "pointerup"); // forward come up
         } else {
            ResetHitTest();
            e.preventDefault();
         }
...
function StandardCSS(Element, TopFlag, PosT, LeftFlag, PosL, DimY, DimX) {
   /*
...
function E3_DisplayNPC(Obj, Func) {
   ...
   */
   ...
   StandardCSS(DivImage, "top", `${Obj.PosY}px`, FlagX, `${PositionX}px`, `${Obj.AtImage}px`, `${Obj.LargImage}px`);
   ...
   if (Obj.PositionText == "Down") StandardCSS(DivText1, "top", `${Obj.PosY + Obj.AtImage + Obj.AltText}px`, FlagX, `${PositionX}px`, "20px", `${Obj.LargText}px`);
   if (Obj.PositionText == "Side") StandardCSS(DivText1, "top", `${Obj.PosY + Obj.AltText}px`, FlagX, `${PositionX + Obj.LargImage}px`, "20px", `${Obj.LargText}px`);
   ...
   StandardCSS(DivBar, "top", `${Obj.PosY}px`, FlagX, `${PositionX}px`, "10px", `${Obj.LargImage}px`);

   const DivBar1 = document.createElement("div");
   DivBar1.style.backgroundColor = "#808080";
   StandardCSS(DivBar1, "top", `${Obj.PosY}px`, FlagX, `${PositionX}px`, "10px", `${Obj.LargImage}px`);
...
function E3_DisplayNPCSingleButton(Obj, Func1) {
   ...
   */
   ...
   if (Obj.PositionText == "Down") StandardCSS(Puls1, "top", `${Obj.PosY + Obj.AtImage + Obj.AltText}px`, FlagX, `${PositionX}px`, `${Obj.AltPuls}px`, `${Obj.LargText}px`);
   if (Obj.PositionText == "Side") StandardCSS(Puls1, "top", `${Obj.PosY + Obj.AltText}px`, FlagX, `${PositionX + Obj.LargImage}px`, `${Obj.AltPuls}px`, `${Obj.LargText}px`);
...
function E3_DisplayNPCDoubleButton(Obj, Func1, Func2) {
   ...
   */
   ...
   //PULSANTE SX
   const Puls1 = document.createElement("div");
   if (Obj.PositionText == "Down") StandardCSS(Puls1, "top", `${Obj.PosY + Obj.AtImage + Obj.AltText}px`, FlagX, `${PositionX}px`, `${Obj.AltPuls}px`, `${Obj.LargText / 2}px`);
   if (Obj.PositionText == "Side") StandardCSS(Puls1, "top", `${Obj.PosY + Obj.AltText}px`, FlagX, `${PositionX + Obj.LargImage}px`, `${Obj.AltPuls}px`, `${Obj.LargText / 2}px`);
   Puls1.style.backgroundColor = Obj.ColorPuls;
   Puls1.style.color = Obj.ColorFontPuls;
   Puls1.style.fontSize = Obj.Font + "px";
   Puls1.style.textAlign = "center";
   ...
   if (Obj.PositionText == "Down") StandardCSS(Puls2, "top", `${Obj.PosY + Obj.AtImage + Obj.AltText}px`, FlagX, `${PositionX + Obj.LargText / 2}px`, `${Obj.AltPuls}px`, `${Obj.LargText / 2}px`);
   if (Obj.PositionText == "Side") StandardCSS(Puls2, "top", `${Obj.PosY + Obj.AltText}px`, FlagX, `${PositionX + Obj.LargImage + Obj.LargText / 2}px`, `${Obj.AltPuls}px`, `${Obj.LargText / 2}px`);
...
function E3_UpdateText(el, text) {
   // Se l'elemento non ha ancora un nodo di testo, lo crea una sola volta
   if (!el.firstChild) {
      el.appendChild(document.createTextNode(""));
   };
   // Aggiorna direttamente il valore del nodo di testo
...
function E3_ModifyArray(Array) {
   ...
   */
   ...
   return { Add, Sub, SwitchUp, SwitchDown, Replace, UpdateArray };
};
...
/*----------------------------------------------------OPTION MENU---------------------------------------------------*/
// export function S0_OptionMenu(Obj) {
//    /*LOCAL STORAGE*/
//    //Language, Graphic, Texture, FPS, InvAxe, RegAxe, VolumeSounds, VolumeVoice, VolumeMusic

//    //PULSANTI LINGUA
//    // if (Obj.ButtonLanguage) {
//    //    for (let i = 0; i < Obj.ButtonLanguage.length; i++) {
//    //       Obj.ButtonLanguage[i].addEventListener('click', function () {
//    //          Obj.SetLanguage(i);
//    //          window.localStorage.setItem('Language', i);
//    //       });
//    //    };
//    // };

//    // //PULSANTI DETTAGLI GRAFICI
//    // if (Obj.ButtonGraphic) {
//    //    for (let i = 0; i < Obj.ButtonGraphic.length; i++) {
//    //       Obj.ButtonGraphic[i].addEventListener('click', function () {
//    //          Obj.SetGraphic(i);
//    //          window.localStorage.setItem('Graphic', i);
//    //       });
//    //    };
//    // };

//    // //PULSANTI TEXTURE
//    // if (Obj.ButtonTexture) {
//    //    for (let i = 0; i < Obj.ButtonTexture.length; i++) {
//    //       Obj.ButtonTexture[i].addEventListener('click', function () {
//    //          Obj.SetTexture(i);
//    //          window.localStorage.setItem('Texture', i);
//    //       });
//    //    };
//    // };

//    // //PULSANTI LIMITE FRAMERATE
//    // if (Obj.ButtonFps) {
//    //    for (let i = 0; i < Obj.ButtonFps.length; i++) {
//    //       Obj.ButtonFps[i].addEventListener('click', function () {
//    //          if (i == 0) {
//    //             Obj.SetFps(30);
...
//    //          if (i == 1) {
//    //             Obj.SetFps(60);
//    //             window.localStorage.setItem('FPS', 60);
//    //          };
//    //          if (i == 2) {
//    //             Obj.SetFps(0);
//    //             window.localStorage.setItem('FPS', 0);
//    //          };
//    //       });
//    //    };
//    // };

//    // //PULSANTI INVERSIONE ASSI
//    // if (Obj.ButtonInvAxes) {
//    //    for (let i = 0; i < Obj.ButtonInvAxes.length; i++) {
//    //       for (let a = 0; a < 2; a++) {
//    //          Obj.ButtonInvAxes[i][a].addEventListener('click', function () {
//    //             Obj.SetInvAxes(i, a);
//    //             window.localStorage.setItem(`InvAxe${i}`, a);       //SALVA IL VALORE NEL LOCAL STORAGE
//    //          });
//    //       };
//    //    };
//    // };

//    //SLIDER SENSIBILITÀ ASSI
//    if (Obj.SliderAxes) {
//       for (let i = 0; i < Obj.SliderAxes.length; i++) {
//          Obj.SliderAxes[i].addEventListener("change", function () {
//             Obj.SetSliderAxes(i, i);
//             window.localStorage.setItem(`RegAxe${i}`, Obj.SliderAxes[i].value);
//             //NEL CASO DEL THROTTLE A DOPPIO ASSE COPIA IL VALORE SULL'INDICE SUCCESSIVO
//             if (i == 3) {
//                Obj.SetSliderAxes(3, 4);
//                window.localStorage.setItem(`RegAxe${4}`, Obj.SliderAxes[3].value);
//             };
//          });
//       };
//    };

//    //SLIDER VOLUME SOUNDS
//    if (Obj.SliderVolSounds) {
//       Obj.SliderVolSounds.addEventListener("change", function () {
//          Obj.SetSliderVolSounds();
//          window.localStorage.setItem(`VolumeSounds`, Obj.SliderVolSounds.value);
//       });
//    };

//    //SLIDER VOLUME VOICE
//    if (Obj.SliderVolVoice) {
//       Obj.SliderVolVoice.addEventListener("change", function () {
//          Obj.SetSliderVolVoice();
//          window.localStorage.setItem(`VolumeVoice`, Obj.SliderVolVoice.value);
//       });
//    };

//    //SLIDER VOLUME MUSIC
//    if (Obj.SliderVolMusic) {
//       Obj.SliderVolMusic.addEventListener("change", function () {
//          Obj.SetSliderVolMusic();
//          window.localStorage.setItem(`VolumeMusic`, Obj.SliderVolMusic.value);
//       });
//    };
...
// function E3_SetWorldPos(Vec3, Obj) {
//    Obj.updateMatrixWorld();
//    /*SETTA UN VETTORE CON LE COORDINATE WORLD DI UN ALTRO OGGETTO*/
//    Vec3.set(0, 0, 0).applyMatrix4(Obj.matrixWorld);
// };
...
function DeepRendererProfiler(renderer, scene, camera, options = {}) {
   ...
   const refreshRate = options.refreshRate || 1000; // ms
   let lastConsoleTime = performance.now();
   let lastFrameTime = performance.now();
   let frame = 0;
   let fps = 0;

   // Funzioni opzionali da profilare
   ...
   // Funzione per stimare il "GPU cost" di una mesh
   function estimateGPUTime(mesh) {
      const tris = mesh.geometry?.index?.count / 3 || mesh.geometry?.attributes?.position?.count / 3 || 0;
      const drawCalls = 1; // una mesh = una draw call
      return tris * drawCalls / 100000; // stima arbitraria in ms
   }
...
function E4_DisableMatrixAutoUpdate(obj, applyToSelf = true, applyToChildren = true) {
   ...
   if (applyToSelf) {
      ...
      obj.updateMatrixWorld = function () { }; // non fa nulla
...
function E3_UpdateProgress(End = false) {
   ...
   //TESTO URL
   if (Gamecharge == 0) {
      //PROMISES GEOMETRIE/OGGETTI 3D
      // if (ActualGeomPromises > 0 && TotalGeomPromises > 0 && ActualGeomPromises < TotalGeomPromises) LoaderScreen.LoaderFile.innerText = `Geom ${PromiseName}`;
      // // if(PromiseName)LoaderScreen.LoaderFile.innerText = `Geom ${PromiseName}`;
      // //MODULI
      // else if (ActualModules > 0 && TotalModules > 0 && ActualModules < TotalModules) LoaderScreen.LoaderFile.innerText = UrlModule;

      LoaderScreen.LoaderFile.innerText = `${PromiseName}`;
   }
...
function E0_ModularShip() {
   ...
   */
   ...
   // disableMatrixAutoUpdate(UserModel );
   ...
   setInterval(() => {
      ...
      if (VarPlanetSystem.PlanetOrbit > 0)
         for (let i = 0; i < VarPlanetSystem.NumMoons; i++) {
            ...
            // E3_UpdateText(DynamHud.children[1].children[i].children[1], Oggetti.PlanetarySystem.Modular[VarPlanetSystem.PlanetOrbit - 1].Modular[i].Name[Language]);
            // DynamHud.children[1].children[i].children[1].innerText =
            //    `${Oggetti.PlanetarySystem.Modular[VarPlanetSystem.PlanetOrbit - 1].Modular[i].Name[Language]}`;
         };
      ...
      if (VarPlanetSystem.MoonOrbit > 0)
         for (let i = 0; i < VarPlanetSystem.NumSubMoons; i++) {
            ...
            //DYNAMIC HUD - NOME INDICATORE
            // if (VarPlanetSystem.MoonOrbit > 0)
            //    E3_UpdateText(DynamHud.children[2].children[0].children[1], Oggetti.PlanetarySystem.Modular[VarPlanetSystem.PlanetOrbit - 1].Modular[VarPlanetSystem.MoonOrbit - 1]
            //       .Modular[i].Name[Language]);
      ...
      if (VarPlanetSystem.DestinationMoon == true) {
         ...
         //-----------------------------DYNAMIC HUD--------------------------------------//
         // E3_UpdateText(DynamHud.children[3].children[0].children[1], Text);
         //DynamHud.children[3].children[0].children[1].innerText = Text;
      };
      //DESTINAZIONE VERSO UNA SUB-LUNA
...
async function E2_ChangeTexturePlanet(Obj) {
   ...
   if (Obj.RingTexture == "") Obj.Mesh.children[2].visible = false;
      ...
      Obj.Mesh.children[2].material.map = await E2_LoadEditTexture(null, `${Obj.Directory}${Obj.RingTexture}${Obj.TypeImage}`, true);
...
async function E0_DynamicPlanetarySystem() {
   ...
   //------------------------------------------AGGIUNTA ORBITE PIANETI E LUNE----------------------------------------//
   for (let i = 0; i < VarPlanetSystem.PlanetsNum; i++) {
   ...
   const UpdatePlanSysPromiseExecution = new OnceFunction(async function () {
      await ObjCreateObj.PlanSysPromiseExecution(VarPlanetSystem.NearPlanetIndex - 1);
   ...
   /*CICLO VELOCE (100MS)*/
   /*ROTAZIONE, CALCOLO DISTANZE E POSIZIONI WORLD, LOD E VISIBILITÀ STAZIONI, TEMPI DALLA NAVE SPAZIALE, ISTANZA RAGGIO TRAENTE, COLLISIONI*/
   setInterval(async () => {
      if (VarPlanetSystem.OrbitOnceLoaded == true) {
         ...
         //DISTANZA DAL SOLE
         VarPlanetSystem.IndDist[0] = UserPosWorld.distanceTo(PosZero) / 1000;
         ...
         if (VarPlanetSystem.NearPlanetIndex == 0)
            for (let i = 0; i < VarPlanetSystem.PlanetsNum; i++) {      //PER OGNI PIANETA ESCLUSA LA STELLA MADRE
               // E3_SetWorldPos(VarPlanetSystem.WorldPosPlanets[i], VarPlanetSystem.References[i].SeasonRot);
         ...
         for (let i = 0; i < VarPlanetSystem.PlanetsNum; i++) {      //PER OGNI PIANETA ESCLUSA LA STELLA MADRE
            ...
            VarPlanetSystem.IndDist[i + 1] = UserPosWorld.distanceTo(VarPlanetSystem.WorldPosPlanets[i]) / 1000;
         };
         ...
         // E3_SetWorldPos(VarPlanetSystem.WorldPosPlanets[VarPlanetSystem.NearPlanetIndex - 1], VarPlanetSystem.References[VarPlanetSystem.NearPlanetIndex - 1].SeasonRot);
         ...
         if (VarPlanetSystem.PlanetOrbit > 0) {
            ...
            for (let a = 0; a < VarPlanetSystem.NumMajorMoons; a++) {
               ...
               if (VarPlanetSystem.NearPlanetIndex && a < VarPlanetSystem.NumMoons) {
                  // Planetary.children[VarPlanetSystem.NearPlanetIndex].children[0].children[a + 1].children[0].getWorldPosition(VarPlanetSystem.WorldPosMoons[a]);

                  VarPlanetSystem.References[VarPlanetSystem.NearPlanetIndex - 1][a].DayRot.getWorldPosition(VarPlanetSystem.WorldPosMoons[a]);

                  // E3_SetWorldPos(VarPlanetSystem.WorldPosMoons[a], VarPlanetSystem.References[VarPlanetSystem.NearPlanetIndex - 1][a].DayRot);
                  VarPlanetSystem.IndMoonDist[a] = UserPosWorld.distanceTo(VarPlanetSystem.WorldPosMoons[a]) / 1000;
               }
               ...
               if (VarPlanetSystem.MoonOrbit > 0) {
                  ...
                  for (let b = 0; b < VarPlanetSystem.NumMajorSubMoons; b++) {
                     if (b < VarPlanetSystem.NumSubMoons) {
                        ...
                        // E3_SetWorldPos(VarPlanetSystem.WorldPosSubMoons[b], VarPlanetSystem.References[VarPlanetSystem.PlanetOrbit - 1][VarPlanetSystem.MoonOrbit - 1][b].DayRot);
                        VarPlanetSystem.IndSubMoonDist[b] = UserPosWorld.distanceTo(VarPlanetSystem.WorldPosSubMoons[b]) / 1000;
                     }
                     //INSERIRE UN VALORE ALTO PER ESCLUDERE GLI ALTRI VALORI
                     else {
                  ...
                  if (VarPlanetSystem.NumSubMoons > 0) {
                     ...
                     //DIAMETRO DELLA SUB-LUNA PIÙ VICINA
                     // VarPlanetSystem.NearSubMoonDiameter = Oggetti.PlanetarySystem.Modular[VarPlanetSystem.PlanetOrbit - 1]
                     //    .Modular[VarPlanetSystem.MoonOrbit - 1].Modular[VarPlanetSystem.NearSubMoonIndex].ScaleXZ * 1000;
                  };
   ...
   setTimeout(() => {
      setInterval(async () => {
         ...
         // VarPlanetSystem.OrbitPosition += VarPlanetSystem.CoeffRot;
      }, 100);
   }, 10000);
   ...
   setTimeout(() => {
      setInterval(() => {
         ...
         //ORBITA DI UNA SUB-LUNA (SE NON È UNA STAZIONE SPAZIALE)
         // else if (VarPlanetSystem.PlanetOrbit > 0 && VarPlanetSystem.MoonOrbit > 0 && VarPlanetSystem.SubMoonOrbit > 0 &&
         //    Oggetti.PlanetarySystem.Modular[VarPlanetSystem.PlanetOrbit - 1].Modular[VarPlanetSystem.MoonOrbit - 1]
         //       .Modular[VarPlanetSystem.SubMoonOrbit - 1].Type == 0) {
         //    //LIMITE DI VELOCITÀ
         //    if ((VarPlanetSystem.NearSubMoonDist - VarPlanetSystem.NearSubMoonDiameter / 1000) < Par.PlanetarySystem.Parametri.LimitDist * (VarPlanetSystem.NearSubMoonDiameter / 1000))
         //       VarPlanetSystem.LimitCollision = (VarPlanetSystem.NearSubMoonDist - VarPlanetSystem.NearSubMoonDiameter / 1000) / (Par.PlanetarySystem.Parametri.LimitDist * (VarPlanetSystem.NearSubMoonDiameter / 1000));
         //    else VarPlanetSystem.LimitCollision = 1;
         //    //ATTRITO CON ATMOSFERA
         //    if ((VarPlanetSystem.NearSubMoonDist - VarPlanetSystem.NearSubMoonDiameter / 1000) < Par.PlanetarySystem.Parametri.CollisionDist * (VarPlanetSystem.NearSubMoonDiameter / 1000))
         //       VarPlanetSystem.NearCollision = true;
         //    else VarPlanetSystem.NearCollision = false;
         //    //COLLISIONE
         //    if ((VarPlanetSystem.NearSubMoonDist - VarPlanetSystem.NearSubMoonDiameter / 1000) <= 0)
         //       VarPlanetSystem.Collision = true;
         //    else VarPlanetSystem.Collision = false;
         // };
      }, 100);
   ...
   setInterval(() => {
      //POSIZIONI WORLD DEI PIANETI ESCLUSO IL PIÙ VICINO E LA STELLA MADRE
      for (let i = 0; i < VarPlanetSystem.PlanetsNum; i++) {      //PER OGNI PIANETA ESCLUSA LA STELLA MADRE
         //VarPlanetSystem.References[i].SeasonRot.getWorldPosition(VarPlanetSystem.WorldPosPlanets[i]);
         // E3_SetWorldPos(VarPlanetSystem.WorldPosPlanets[i], VarPlanetSystem.References[i].SeasonRot);
      };
...
/*--------------------HYPERLOOP-------------------------------*/
//#region
const DustPos = [];      //POSIZIONE Z VERTICI

// function DynamicDust() {
//    if (Par.Log.Moduli == true) console.log("DynamicDust");
//    const Dust = new THREE.LineSegments();
//    Dust.name = "Dust";
//    const DustVertices = [];

//    for (let i = 0; i < Par.DynamicDust.Number; i++) {
//       DustPos[i] = Math.random() * Par.DynamicDust.CloudSizeZ;
//       const x = Math.random() * Par.DynamicDust.CloudSizeX;
//       const y = Math.random() * Par.DynamicDust.CloudSizeY;

//       //INSERIRE I VERTICI SE SONO FUORI DALLA PARTE VUOTA IMPOSTATA
//       if ((x < Par.DynamicDust.CloudSizeX / 2 - Par.DynamicDust.EmptySizeX / 2 ||
//          x > Par.DynamicDust.CloudSizeX / 2 + Par.DynamicDust.EmptySizeX / 2) ||
//          (y < Par.DynamicDust.CloudSizeY / 2 - Par.DynamicDust.EmptySizeY / 2 ||
//             y > Par.DynamicDust.CloudSizeY / 2 + Par.DynamicDust.EmptySizeY / 2)) {
//          DustVertices.push(x, y, DustPos[i]);
//          DustVertices.push(x, y, DustPos[i]);
//       };
//    };

//    const DustGeom = new THREE.BufferGeometry();
//    DustGeom.setAttribute('position', new THREE.Float32BufferAttribute(DustVertices, 3));

//    const DustMat = new THREE.LineBasicMaterial({
//       color: Par.DynamicDust.Color,
//       transparent: true,
//       opacity: Par.DynamicDust.Opacity,
//    });

//    Dust.geometry = DustGeom;
//    Dust.material = DustMat;

//    //POSIZIONAMENTO DI FRONTE ALLA NAVE SPAZIALE
//    Dust.position.set(-Par.DynamicDust.CloudSizeX / 2, -Par.DynamicDust.CloudSizeY / 2, -Par.DynamicDust.CloudSizeZ);
//    Dust.visible = false;
...
//          (Math.random() - 0.5) * Lato,
//          (Math.random() - 0.5) * Lato,
//          (Math.random() - 0.5) * Lato
//       ));
//    };

//    // direzione unica per tutte le linee
//    const euler = new THREE.Euler(0, 0, 0);

//    const positions = new Float32Array(lineCount * 6); // start + end per linea
//    const geometry = new THREE.BufferGeometry();
//    geometry.setAttribute('position', new THREE.BufferAttribute(positions, 3));
...
//    // -------------------------------------------------------
//    // UPDATE: definisci qui start di ogni linea, lunghezza e direzione unica
//    // -------------------------------------------------------
//    function Update(Obj) {
//       // points: array di THREE.Vector3, lunghezza = lineCount
//       // euler: unica rotazione per tutte le linee
//       // length: lunghezza delle linee

//       // calcola la direzione una sola volta
//       dir.set(0, 0, 1);
...
//          positions[idx + 3] = start.x + dir.x * Obj.Length;
//          positions[idx + 4] = start.y + dir.y * Obj.Length;
//          positions[idx + 5] = start.z + dir.z * Obj.Length;
//       }
...
//    // -------------------------------------------------------
//    // UPDATE: muove le linee lungo la direzione della nave
//    // -------------------------------------------------------
//    function Update(Obj, delta) {
//       // Obj.Quaternion -> orientamento della nave
//       // Obj.Length -> lunghezza delle linee
//       // Obj.Speed -> velocità costante

//       // calcola la direzione una sola volta
//       dir.set(0, 0, 1);
//       dir.applyQuaternion(Obj.Quaternion).normalize();

//       const move = Obj.Speed * delta;
...
//          // se la linea esce dal cubo, rigenerala dietro
//          if (Math.abs(start.x) > Lato / 2 || Math.abs(start.y) > Lato / 2 || Math.abs(start.z) > Lato / 2) {
//             start.x = (Math.random() - 0.5) * Lato;
//             start.y = (Math.random() - 0.5) * Lato;
//             start.z = (Math.random() - 0.5) * Lato;
//          }
...
function Hyperloop(Object) {
   ...
   function Update(Obj, delta) {
      ...
      for (let i = 0; i < Object.Number; i++) {
         ...
         // se la linea è troppo lontana davanti, riportala dietro
         // consideriamo "dietro" come -Lato/2 lungo la direzione locale
...
function E2_GenerateGeneric(Num) {
   ...
   ImportedObject.clear();
   Scene.remove(ImportedObject);
   //ImportedObject.copy(Oggetti3D.Generic.Model[Num]);
   ...
   //OGGETTO 3D ABILITATO
   // if (Oggetti.Generic.Modular[Num].Mesh == true) {
   //    ImportedObject = new THREE.Group();
   //    ImportedObject.copy(Oggetti3D.Generic.Model[Num]);
   // };
   // if (Oggetti.Generic.Modular[Num].UniversalGeom == true) {
   //    const Materials = [];
   //    //CREAZIONE ARRAY DI MATERIALI
   //    for (let i = 0; i < Geometrie[Oggetti.Generic.Modular[Num].GeomModel].Multi.length; i++) {
   //       Materials[i] = MaterialArray[Geometrie[Oggetti.Generic.Modular[Num].GeomModel].Multi[i].Material];
   //    };
   //    //SOLO GEOMETRIA INDICIZZATA
   //    if (Oggetti.Generic.Modular[Num].Mesh == false) {     //SE NON ESISTE IL MODELLO 3D NELL'OGGETTO "Oggetti3D"
   //       ImportedObject = new THREE.Mesh(UniversalGeom[Geometrie[Oggetti.Generic.Modular[Num].GeomModel].Varianti[Oggetti.Generic.Modular[Num].Variante].Indice], Materials);
   //       ImportedObject.name = "MultiUniversalGeom";
   //    }
   //    //IBRIDO OGGETTO 3D E GEOMETRIA INDICIZZATA
   //    else {
   //       const NewMesh = new THREE.Mesh(UniversalGeom[Geometrie[Oggetti.Generic.Modular[Num].GeomModel].Varianti[Oggetti.Generic.Modular[Num].Variante].Indice], Materials);
   //       NewMesh.name = "MultiUniversalGeom";
   //       ImportedObject.add(NewMesh);
   //    };
   // };
   ...
   //SCALA
   //ImportedObject.scale.setScalar(Oggetti.Generic.Modular[Num].EditorScale);
...
async function CreateObj(Obj, Oggetti, Geometrie, Materiali, Par, manager) {
   ...
   const promises = Materiali.map(async (mat, h) => {
      ...
      if (mat.Type == "Punti") m = E3_MaterialePunti(mat); // sincrono
   ...
   // const MaterialArray = await Promise.all(promises);
   MaterialArray = await Promise.all(promises);
   // console.log(`Materiali: ${MaterialArray.length}`);
   ...
   async function Generic2(Dir, Num, Name, GeomModel, VariabiliObj, Tractor, UniversalGeom) {
      ...
      //if (Dir == "Generic") GenericGroup.add(SubMeshGroup);
               ...
               if (x < NumVisible) {
                  for (let c = 0; c < Geometrie[GeomModel].Multi[a].Geometry[b].GeomArray.length; c++) {
                     ...
                     if (Geometrie[GeomModel].Multi[a].Geometry[b].GeomArray[c].Group == false) {
                        ...
                        // if (Geometrie[GeomModel].Multi[a].Geometry[b].GeomArray[c].Option == "Number") {
                        //    //CERCA NELLE VARIABILI IL NUMERO
                        //    for (let i = 0; i < Geometrie[GeomModel].Variabili.Option.length; i++) {
                        //       if (Geometrie[GeomModel].Variabili.Option[i].length == 4 && Geometrie[GeomModel].Variabili.Option[i][0] == a &&
                        //          Geometrie[GeomModel].Variabili.Option[i][1] == b && Geometrie[GeomModel].Variabili.Option[i][2] == c &&
                        //          Geometrie[GeomModel].Variabili.Option[i][4] == x) {
                        //          GeomArraySubGroup.push(GenerateGeometry(Geometrie[GeomModel].Multi[a].Geometry[b].GeomArray[c]));
                        //       };
                        //    };
                        // };
                     ...
                     if (Geometrie[GeomModel].Multi[a].Geometry[b].GeomArray[c].Group == true) {
                        ...
                        //    for (let i = 0; i < Geometrie[GeomModel].Variabili.Option.length; i++) {
                        //       if (Geometrie[GeomModel].Variabili.Option[i].length == 4 && Geometrie[GeomModel].Variabili.Option[i][0] == a &&
                        //          Geometrie[GeomModel].Variabili.Option[i][1] == b &&
                        //          Geometrie[GeomModel].Variabili.Option[i][2] == c && Geometrie[GeomModel].Variabili.Option[i][4] == x) {
                        //          //GeomArraySubGroup.push(GenerateGeometry(Geometrie[GeomModel].Multi[a].Geometry[b].GeomArray[c]));
                        //          //GENERA IL GRUPPO DI LIVELLO C E LE GEOMETRIE DI LIVELLO D
                        //          const GeomGroupC = GenerateMultiGroupLevelC(a, b, c, x);
                        //          GeomArraySubGroup.push(GeomGroupC);
               ...
               if (y < NumVisible) {
                  for (let d = 0; d < Geometrie[GeomModel].Multi[a].Geometry[b].GeomArray[c].GeomArray.length; d++) {
                     ...
                     if (Geometrie[GeomModel].Multi[a].Geometry[b].GeomArray[c].GeomArray[d].Group == false) {
                        ...
                        //SE È OPZIONABILE COME NUMERO
                        // if (Geometrie[GeomModel].Multi[a].Geometry[b].GeomArray[c].GeomArray[d].Option == "Number") {
                        //    console.log("CIAO");
                        ...
                        //          Geometrie[GeomModel].Variabili.Option[i][1] == b && Geometrie[GeomModel].Variabili.Option[i][2] == c &&
                        //          Geometrie[GeomModel].Variabili.Option[i][3] == d && Geometrie[GeomModel].Variabili.Option[i][4] == x &&
                        //          Geometrie[GeomModel].Variabili.Option[i][5] == y) {
                        //          GeomArraySubGroup.push(GenerateGeometry(Geometrie[GeomModel].Multi[a].Geometry[b].GeomArray[c].GeomArray[d]));
                        //       };
               ...
               if (z < NumVisible) {
                  for (let e = 0; e < Geometrie[GeomModel].Multi[a].Geometry[b].GeomArray[c].GeomArray[d].GeomArray.length; e++) {
                     ...
                     if (Geometrie[GeomModel].Multi[a].Geometry[b].GeomArray[c].GeomArray[d].GeomArray[e].Group == false) {
                        ...
                        //SE È OPZIONABILE COME NUMERO
                        // if (Geometrie[GeomModel].Multi[a].Geometry[b].GeomArray[c].GeomArray[d].GeomArray[e].Option == "Number") {
                        //    console.log("CIAO");
                        //    //CERCA NELLA VARIABILI SE L'OPZIONE È TRUE
                        //    for (let i = 0; i < Geometrie[GeomModel].Variabili.Option.length; i++) {
                        //       if (Geometrie[GeomModel].Variabili.Option[i].length == 8 && Geometrie[GeomModel].Variabili.Option[i][0] == a &&
                        //          Geometrie[GeomModel].Variabili.Option[i][1] == b && Geometrie[GeomModel].Variabili.Option[i][2] == c &&
                        //          Geometrie[GeomModel].Variabili.Option[i][3] == d && Geometrie[GeomModel].Variabili.Option[i][4] == e &&
                        //          Geometrie[GeomModel].Variabili.Option[i][5] == x && Geometrie[GeomModel].Variabili.Option[i][6] == y &&
                        //          Geometrie[GeomModel].Variabili.Option[i][7] == z) {
                        //          GeomArraySubGroup.push(GenerateGeometry(Geometrie[GeomModel].Multi[a].Geometry[b].GeomArray[c].GeomArray[d].GeomArray[e]));
                        //       };
            ...
            if (x < NumVisible) {
               for (let c = 0; c < Geometrie[GeomModel].Generic[a].Geometry[b].GeomArray.length; c++) {
                  ...
                  if (Geometrie[GeomModel].Generic[a].Geometry[b].GeomArray[c].Group == false) {
                     ...
                     //SE È OPZIONABILE COME NUMERO
                     // if (Geometrie[GeomModel].Generic[a].Geometry[b].GeomArray[c].Option == "Number") {
                     //    console.log("CIAO");
                     //    //CERCA NELLE VARIABILI IL NUMERO
                     //    for (let i = 0; i < Geometrie[GeomModel].Variabili.Option.length; i++) {
                     //       if (Geometrie[GeomModel].Variabili.Option[i].length == 4 && Geometrie[GeomModel].Variabili.Option[i][0] == a &&
            ...
            if (y < NumVisible) {
               for (let d = 0; d < Geometrie[GeomModel].Generic[a].Geometry[b].GeomArray[c].GeomArray.length; d++) {
                  ...
                  if (Geometrie[GeomModel].Generic[a].Geometry[b].GeomArray[c].GeomArray[d].Group == false) {
                     ...
                     //SE È OPZIONABILE COME NUMERO
                     // if (Geometrie[GeomModel].Generic[a].Geometry[b].GeomArray[c].GeomArray[d].Option == "Number") {
                     //    console.log("CIAO");
                     //    //CERCA NELLA VARIABILI SE L'OPZIONE È TRUE
                     //    for (let i = 0; i < Geometrie[GeomModel].Variabili.Option.length; i++) {
                     //       if (Geometrie[GeomModel].Variabili.Option[i].length == 6 && Geometrie[GeomModel].Variabili.Option[i][0] == a &&
            ...
            if (z < NumVisible) {
               for (let e = 0; e < Geometrie[GeomModel].Generic[a].Geometry[b].GeomArray[c].GeomArray[d].GeomArray.length; e++) {
                  ...
                  if (Geometrie[GeomModel].Generic[a].Geometry[b].GeomArray[c].GeomArray[d].GeomArray[e].Group == false) {
                     ...
                     //SE È OPZIONABILE COME NUMERO
                     // if (Geometrie[GeomModel].Generic[a].Geometry[b].GeomArray[c].GeomArray[d].GeomArray[e].Option == "Number") {
                     //    console.log("CIAO");
                     //    //CERCA NELLA VARIABILI SE L'OPZIONE È TRUE
                     //    for (let i = 0; i < Geometrie[GeomModel].Variabili.Option.length; i++) {
                     //       if (Geometrie[GeomModel].Variabili.Option[i].length == 8 && Geometrie[GeomModel].Variabili.Option[i][0] == a &&
      ...
      // Disattiva l'aggiornamento automatico ovunque
      // SubMeshGroup.matrixAutoUpdate = true;
      // SubMeshGroup.traverse(child => {
      //    if (child !== SubMeshGroup) {
      //       child.matrixAutoUpdate = false;
      //    }
      // });

      // // Aggiorna solo la matrice locale e globale del gruppo
      // SubMeshGroup.updateMatrix();         // Aggiorna la matrix locale
      // SubMeshGroup.updateMatrixWorld(false); // NON ricorsivo!
            ...
            if (Geometrie[GeomModel].MeshMulti[a].Group == false) {
               if (Geometrie[GeomModel].MeshMulti[a].Option == true) {
                  ...
                  if (typeof Geometrie[GeomModel].MeshMulti[a].Material === 'number') {
                     // E3_GenMesh(MultiMeshGroup, MeshGeometry, MaterialArray[Geometrie[GeomModel].MeshMulti[a].Material],
                     //    Geometrie[GeomModel].MeshMulti[a].Position,
                     //    Geometrie[GeomModel].MeshMulti[a].Rotation,
                     //    Geometrie[GeomModel].MeshMulti[a].Scale,
                     //    "", true, Geometrie[GeomModel].MeshMulti[a].Shadows);
                     E3_UniversalMesh({
                        //PARAMETRI OBBLIGATORI:
                        Geom: MeshGeometry,
                  ...
                  //MATERIALE STAMPDOCK
                     ...
                     // E3_GenMesh(MultiMeshGroup, MeshGeometry, Stamp,
                     //    Geometrie[GeomModel].MeshMulti[a].Position,
                     //    Geometrie[GeomModel].MeshMulti[a].Rotation,
                     //    Geometrie[GeomModel].MeshMulti[a].Scale,
                     //    "", true, Geometrie[GeomModel].MeshMulti[a].Shadows);
               ...
               if (Geometrie[GeomModel].MeshMulti[a].MeshArray[b].Group == false) {
                  if (Geometrie[GeomModel].MeshMulti[a].MeshArray[b].Option == true) {
                     ...
                     if (typeof Geometrie[GeomModel].MeshMulti[a].MeshArray[b].Material === 'number') {
                        // E3_GenMesh(MeshSubGroup, MeshGeometry, MaterialArray[Geometrie[GeomModel].MeshMulti[a].MeshArray[b].Material],
                        //    Geometrie[GeomModel].MeshMulti[a].MeshArray[b].Position,
                        //    Geometrie[GeomModel].MeshMulti[a].MeshArray[b].Rotation,
                        //    Geometrie[GeomModel].MeshMulti[a].MeshArray[b].Scale,
                        //    Geometrie[GeomModel].MeshMulti[a].MeshArray[b].Name, true, false);
                        E3_UniversalMesh({
                           //PARAMETRI OBBLIGATORI:
                           Geom: MeshGeometry,
                        ...
                        // E3_GenMesh(MeshSubGroup, MeshGeometry, Stamp,
                        //    Geometrie[GeomModel].MeshMulti[a].MeshArray[b].Position,
                        //    Geometrie[GeomModel].MeshMulti[a].MeshArray[b].Rotation,
                        //    Geometrie[GeomModel].MeshMulti[a].MeshArray[b].Scale,
                        //    Geometrie[GeomModel].MeshMulti[a].MeshArray[b].Name, true, false);
               ...
               if (Geometrie[GeomModel].MeshMulti[a].MeshArray[b].MeshArray[c].Group == false) {
                  if (Geometrie[GeomModel].MeshMulti[a].MeshArray[b].MeshArray[c].Option == true) {
                     ...
                     if (typeof Geometrie[GeomModel].MeshMulti[a].MeshArray[b].MeshArray[c].Material === 'number') {
                        // E3_GenMesh(MeshSubGroup, MeshGeometry, MaterialArray[Geometrie[GeomModel].MeshMulti[a].MeshArray[b].MeshArray[c].Material],
                        //    Geometrie[GeomModel].MeshMulti[a].MeshArray[b].MeshArray[c].Position,
                        //    Geometrie[GeomModel].MeshMulti[a].MeshArray[b].MeshArray[c].Rotation,
                        //    Geometrie[GeomModel].MeshMulti[a].MeshArray[b].MeshArray[c].Scale,
                        //    Geometrie[GeomModel].MeshMulti[a].MeshArray[b].MeshArray[c].Name, true, false);
                        E3_UniversalMesh({
                           //PARAMETRI OBBLIGATORI:
                           Geom: MeshGeometry,
            ...
            if (Geometrie[GeomModel].MeshGroup[a].Group == false) {
               if (Geometrie[GeomModel].MeshGroup[a].Option == true) {
                  ...
                  if (typeof Geometrie[GeomModel].MeshGroup[a].Material === 'number') {
                     // E3_GenMesh(SubMeshGroup, MeshGeometry, MaterialArray[Geometrie[GeomModel].MeshGroup[a].Material],
                     //    Geometrie[GeomModel].MeshGroup[a].Position,
                     //    Geometrie[GeomModel].MeshGroup[a].Rotation,
                     //    Geometrie[GeomModel].MeshGroup[a].Scale,
                     //    Geometrie[GeomModel].MeshGroup[a].Name, true, Geometrie[GeomModel].MeshGroup[a].Shadows);
                     E3_UniversalMesh({
                        //PARAMETRI OBBLIGATORI:
                        Geom: MeshGeometry,
                     ...
                     // E3_GenMesh(SubMeshGroup, MeshGeometry, Stamp,
                     //    Geometrie[GeomModel].MeshGroup[a].Position,
                     //    Geometrie[GeomModel].MeshGroup[a].Rotation,
                     //    Geometrie[GeomModel].MeshGroup[a].Scale,
                     //    Geometrie[GeomModel].MeshGroup[a].Name, true, Geometrie[GeomModel].MeshGroup[a].Shadows);
               ...
               if (Geometrie[GeomModel].MeshGroup[a].MeshArray[b].Group == false) {
                  if (Geometrie[GeomModel].MeshGroup[a].MeshArray[b].Option == true) {
                     ...
                     if (typeof Geometrie[GeomModel].MeshGroup[a].MeshArray[b].Material === 'number') {
                        // E3_GenMesh(MeshSubGroup, MeshGeometry, MaterialArray[Geometrie[GeomModel].MeshGroup[a].MeshArray[b].Material],
                        //    Geometrie[GeomModel].MeshGroup[a].MeshArray[b].Position,
                        //    Geometrie[GeomModel].MeshGroup[a].MeshArray[b].Rotation,
                        //    Geometrie[GeomModel].MeshGroup[a].MeshArray[b].Scale,
                        //    Geometrie[GeomModel].MeshGroup[a].MeshArray[b].Name, true, Geometrie[GeomModel].MeshGroup[a].MeshArray[b].Shadows);
                        E3_UniversalMesh({
                           //PARAMETRI OBBLIGATORI:
                           Geom: MeshGeometry,
               ...
               if (Geometrie[GeomModel].MeshGroup[a].MeshArray[b].MeshArray[c].Group == false) {
                  if (Geometrie[GeomModel].MeshGroup[a].MeshArray[b].MeshArray[c].Option == true) {
                     ...
                     if (typeof Geometrie[GeomModel].MeshGroup[a].MeshArray[b].MeshArray[c].Material === 'number') {
                        // E3_GenMesh(MeshSubGroup, MeshGeometry, MaterialArray[Geometrie[GeomModel].MeshGroup[a].MeshArray[b].MeshArray[c].Material],
                        //    Geometrie[GeomModel].MeshGroup[a].MeshArray[b].MeshArray[c].Position,
                        //    Geometrie[GeomModel].MeshGroup[a].MeshArray[b].MeshArray[c].Rotation,
                        //    Geometrie[GeomModel].MeshGroup[a].MeshArray[b].MeshArray[c].Scale,
                        //    Geometrie[GeomModel].MeshGroup[a].MeshArray[b].MeshArray[c].Name, true, Geometrie[GeomModel].MeshGroup[a].MeshArray[b].MeshArray[c].Shadows);
                        E3_UniversalMesh({
                           //PARAMETRI OBBLIGATORI:
                           Geom: MeshGeometry,
            ...
            if (x < NumVisible) {
               for (let c = 0; c < Geometrie[GeomModel].Recycled[Model][a + 1].Geometry[b].GeomArray.length; c++) {
                  ...
                  if (Geometrie[GeomModel].Recycled[Model][a + 1].Geometry[b].GeomArray[c].Group == false) {
                     ...
                     //SE È OPZIONABILE COME NUMERO
                     // if (Geometrie[GeomModel].Recycled[Model][a + 1].Geometry[b].GeomArray[c].Option == "Number") {
                     //    console.log("CIAO");
                     //    //CERCA NELLE VARIABILI IL NUMERO
                     //    for (let i = 0; i < Geometrie[GeomModel].Variabili.Option.length; i++) {
                     //       if (Geometrie[GeomModel].Variabili.Option[i].length == 4 && Geometrie[GeomModel].Variabili.Option[i][0] == a &&
            ...
            if (z < NumVisible) {
               for (let e = 0; e < Geometrie[GeomModel].Recycled[Model][a + 1].Geometry[b].GeomArray[c].GeomArray[d].GeomArray.length; e++) {
                  ...
                  if (Geometrie[GeomModel].Recycled[Model][a + 1].Geometry[b].GeomArray[c].GeomArray[d].GeomArray[e].Group == false) {
                     ...
                     //SE È OPZIONABILE COME NUMERO
                     // if (Geometrie[GeomModel].Recycled[Model][a + 1].Geometry[b].GeomArray[c].GeomArray[d].GeomArray[e].Option == "Number") {
                     //    console.log("CIAO");
                     //    //CERCA NELLA VARIABILI SE L'OPZIONE È TRUE
                     //    for (let i = 0; i < Geometrie[GeomModel].Variabili.Option.length; i++) {
                     //       if (Geometrie[GeomModel].Variabili.Option[i].length == 8 && Geometrie[GeomModel].Variabili.Option[i][0] == a &&
                  ...
                  if (Geometrie[GeomModel].Recycled[Model][a + 1].Geometry[b].GeomArray[c].GeomArray[d].GeomArray[e].Group == true) {
                     ...
                     //SE È OPZIONABILE COME NUMERO
                     // if (Geometrie[GeomModel].Recycled[Model][a + 1].Geometry[b].GeomArray[c].GeomArray[d].Option == "Number") {
                     //    console.log("CIAO");
                     //    //CERCA NELLA VARIABILI SE L'OPZIONE È TRUE
                     //    for (let i = 0; i < Geometrie[GeomModel].Variabili.Option.length; i++) {
                     //       if (Geometrie[GeomModel].Variabili.Option[i].length == 8 && Geometrie[GeomModel].Variabili.Option[i][0] == a &&
   ...
   if (Par.CreateObj.PlanetarySystem == true) {
      ...
      /*---------------------------------PROMISES GEOMETRIE INDICIZZATE--------------------------------------------*/
      //PER OGNI GEOMETRIA
      // for (let i = 0; i < Geometrie.length; i++) {
      //    //SE È INDICIZZATA E SE È ASSOCIATA AL MODULO
      //    if ('Varianti' in Geometrie[i] && 'Parametri' in Geometrie[i]) {
      //       if (Geometrie[i].Parametri.Modulo == "PlanetarySystem")
      //          //PER OGNI VARIANTE
      //          for (let a = 0; a < Geometrie[i].Varianti.length; a++) {
      //             // Promises[PromiseCount] = new Promise((resolve, reject) => {
      //             //    resolve(GenericGeometry(i, a));
      //             // });
      //             Promises[PromiseCount] = () => GenericGeometry(i, a);
      //             Promises[PromiseCount]._name = "Geometria indicizzata Planetary System";
      //             PromiseCount++;
      //          };
      //    };
      // };

      /*------------------------------------PROMISES GEOMETRIE RICICLATE---------------------------------*/
      //PER OGNI GEOMETRIA
      // for (let i = 0; i < Geometrie.length; i++) {
      //    //SE È RICICLATA
      //    if ('Recycled' in Geometrie[i] && 'Parametri' in Geometrie[i]) {
      //       if (Geometrie[i].Parametri.Modulo == "PlanetarySystem")
      //          //PER OGNI OGGETTO RICICLATO
      //          for (let a = 0; a < Geometrie[i].Recycled.length; a++) {
      //             // Promises[PromiseCount] = new Promise((resolve, reject) => {
      //             //    resolve(GenericRecycledGeometry(i, a));
      //             // });
      //             Promises[PromiseCount] = () => GenericRecycledGeometry(i, a);
      //             Promises[PromiseCount]._name = "Geometria riciclata Planetary System";
      //             PromiseCount++;
      //          };
      //    };
      // };
   ...
   if (Par.CreateObj.Spaceship == true) {
      //GENERAZIONE OGGETTO GENERICO
      for (let i = 0; i < Oggetti.Spaceship.Modular.length; i++) {
         // if (!Oggetti.Spaceship.Modular[i].UniversalGeom || Oggetti.Spaceship.Modular[i].UniversalGeom == false)
      ...
      //    //SE È INDICIZZATA E SE È ASSOCIATA AL MODULO
      //    if ('Varianti' in Geometrie[i] && 'Parametri' in Geometrie[i]) {
      //       if (Geometrie[i].Parametri.Modulo == "Spaceship")
      //          //PER OGNI VARIANTE
      //          for (let a = 0; a < Geometrie[i].Varianti.length; a++) {
      //             // Promises[PromiseCount] = new Promise((resolve, reject) => {
      //             //    resolve(GenericGeometry(i, a));
      //             // });
      //             Promises[PromiseCount] = () => GenericGeometry(i, a);
      //             Promises[PromiseCount]._name = "Geometria indicizzata Nave";
      //             PromiseCount++;
      //          };
      //    };
      // };
...
export async function MicEngine(Parameters, OggettiObj, GeometrieObj, MaterialiObj) {
   ...
   if (Par.Renderer.AmbientMap.Enable == true) {
      ...
      if (Par.Renderer.AmbientMap.Type == "hdr") {
         ...
         new HDRLoader().load(Par.Renderer.AmbientMap.HdrDirectory, (tex) => {
            ...
            //Scene.environment.rotation = Math.PI / 2; // funziona nelle versioni recenti
            tex.dispose();
            pmrem.dispose();
         });
      };
   ...
   if (Par.Moduli.CreateObj == true) {
      ObjCreateObj = await CreateObj(Oggetti3D, Oggetti, GeometrieObj, MaterialiObj, Par, Manager);
      await ObjCreateObj.PromiseExecution();
   ...
   for (let i = 0; i < Oggetti.Generic.Modular.length; i++) {
      if (Par.CreateObj.GenericObject[i] == 1) {
         ...
         // let Object;
         // GenericGroup.add(Oggetti3D.Generic.Model[i]);

         // //OGGETTO 3D ABILITATO
         // if (Oggetti.Generic.Modular[i].Mesh == true) {
         //    Object = new THREE.Group();
         //    Object.copy(Oggetti3D.Generic.Model[i]);
         // };
         // if (Oggetti.Generic.Modular[i].UniversalGeom == true) {
         //    const Materials = [];
         //    //CREAZIONE ARRAY DI MATERIALI
         //    for (let a = 0; a < Geometrie[Oggetti.Generic.Modular[i].GeomModel].Multi.length; a++) {
         //       Materials[a] = MaterialArray[Geometrie[Oggetti.Generic.Modular[i].GeomModel].Multi[a].Material];
         //    };
         //    //SOLO GEOMETRIA INDICIZZATA
         //    if (Oggetti.Generic.Modular[i].Mesh == false) {     //SE NON ESISTE IL MODELLO 3D NELL'OGGETTO "Oggetti3D"
         //       Object = new THREE.Mesh(UniversalGeom[Geometrie[Oggetti.Generic.Modular[i].GeomModel].Varianti[Oggetti.Generic.Modular[i].Variante].Indice], Materials);
         //       Object.name = "MultiUniversalGeom";
         //    }
         //    //IBRIDO OGGETTO 3D E GEOMETRIA INDICIZZATA
         //    else {
         //       const NewMesh = new THREE.Mesh(UniversalGeom[Geometrie[Oggetti.Generic.Modular[i].GeomModel].Varianti[Oggetti.Generic.Modular[i].Variante].Indice], Materials);
         //       NewMesh.name = "MultiUniversalGeom";
         //       console.log(NewMesh);
         //       Object.add(NewMesh);
         //    };
   ...
   // VarObjectExport.E3_SetWorldPos = E3_SetWorldPos;
   ...
   if (Par.Moduli.ModularShip == true) {
      ...
      MicEnginereturn.User.UserPosWorld = UserPosWorld;
   ...
   if (Par.Moduli.DynamicCockpit == true) {
      ...
      // DynamHud = document.createElement('div');
      // DynamCockpit = E0_DynamicCockpit(DynamCockpitVar, DynamHud);
      DynamCockpit = E0_DynamicCockpit(DynamCockpitVar);

      //STILE AREA DOM CORNICI
      // StandardCSS(DynamHud, "top", `${Par.DynamicCockpit.Top}%`, "left",
      //   `${(100 - Par.DynamicCockpit.Width) / 2}%`, `${Par.DynamicCockpit.Height}%`, `${Par.DynamicCockpit.Width}%`);

      //ESPORTAZIONE
      // MicEnginereturn.DynamicHud = DynamHud;
      // MicEnginereturn.DynamicHudDir = DynamHudDir;
   ...
   /*--------------------DYNAMIC DUST-------------------------------*/
   // if (Par.Moduli.DynamicDust == true) {
   //    UrlModule = "DynamicDust";
   //    //NUOVO MODO
   //    VarObjectExport.DustPos = DustPos;
   //    MicEnginereturn.DynamicDust = DynamicDust();
   ...
   function E4_ForceMinimalRender(scene, renderer) {
      // 1️⃣ Materiale base per tutti i mesh
      scene.overrideMaterial = new THREE.MeshBasicMaterial({ color: 0x808080 });
      ...
      // 3️⃣ Riduci pixel ratio per mobile
      renderer.setPixelRatio(Math.min(window.devicePixelRatio, 1));
      renderer.setSize(window.innerWidth, window.innerHeight, false);

      // 4️⃣ Disattiva frustum culling
      scene.traverse(obj => {
         if (obj.isMesh) obj.frustumCulled = false;
      });

      // 5️⃣ Disattiva callback di render
      scene.traverse(obj => {
         if (obj.hasOwnProperty('onBeforeRender') && obj.onBeforeRender != null) {
            obj.onBeforeRender = null;
         }
         if (obj.hasOwnProperty('onAfterRender') && obj.onAfterRender != null) {
            obj.onAfterRender = null;
         }
      });
      ...
      renderer.setSize(480, 270, false); // test low res
      ...
      // 7️⃣ Post-processing opzionale (solo se usi composer)
      if (renderer.composer) {
         renderer.composer.passes = [];
      }



   };
   //E4_ForceMinimalRender(Scene, renderer);
   ...
   function animate(time) {
      ...
      //MODULAR SHIP
      if (Par.Moduli.ModularShip == true) GroupUser.getWorldPosition(UserPosWorld);
